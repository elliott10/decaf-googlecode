...
static int func_call_flag=0;
static int func_ret_flag=0;
...
FILE * func_log=DECAF_NULL_HANDLE;

#define MAX_STACK_SIZE 5000

char modname_t[512]="ucore_lab";
char func_name_t[512];

char call_func_name[30];
char ret_func_name[30];
...
void check_call(DECAF_Callback_Params *param)
{
	...
	*/
	...
	if(stack_top > 0)
	{
		...
		if(funcmap_get_name_by_pc_c(sys_call_entry_stack[stack_top-1], modname_t, call_func_name))
		{
			memset(call_func_name, 0, 30);
		}

		DECAF_printf("%-10.10s(0x%-8x)",call_func_name,sys_call_entry_stack[stack_top-1]);
		func_call_flag++;
...
void check_ret(DECAF_Callback_Params *param)
{
	...
	if(param->be.next_pc == sys_call_ret_stack[stack_top-1])
	{
		...
		if(stack_top > 0)
		{
			...
			if(func_call_flag > 0)
			{
				func_ret_flag = func_call_flag-1;
				DECAF_printf("\n");
				for(i=func_ret_flag;i > 0;i--)
					DECAF_printf("                         ");
				...
				for(i=func_ret_flag-1;i > 0;i--)
					DECAF_printf("                         ");

				func_ret_flag--;
			}
			else if(func_call_flag == 0)
			{
				for(i=func_ret_flag;i > 0;i--)
					DECAF_printf("                         ");
			...
			{
				DECAF_printf("\n *** error! func_call_flag:[%d]\n",func_call_flag);
			}

			if(funcmap_get_name_by_pc_c(sys_call_entry_stack[stack_top-1], modname_t, ret_func_name))
			{
				memset(ret_func_name, 0, 30);
			}

			DECAF_printf("   %-10.10s(0x%-8x)<--+\n",ret_func_name,sys_call_entry_stack[stack_top-1]);
			func_call_flag = 0;
...
void func_log_init(void)
{
	/*
	func_log= fopen("./func.log","w");
	if(!func_log)
	{
		DECAF_printf("the %s can not be open or created !!","./func.log");
		return;
	}
	*/
	if(!handle_block_end_cb)
